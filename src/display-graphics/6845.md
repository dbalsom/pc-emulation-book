...
{{#bitfield h3 mc6845_registers.toml#r10-cursor-start}}
...
> [!IMPORTANT]
> The `CA` flag is not updated during the **vertical total adjust** period.
...
The MC6845 has basic support for [light pens](https://en.wikipedia.org/wiki/Light_pen). The chip's physical `LPSTB` or **light pen strobe** pin is a positive-edge-triggered input, that causes the current `VMA` to be written to `R16` and `R17`. This is an asynchronous input signal that can occur any time, so presents a bit of challenge for an otherwise synchronous chip. The exact electrical details are not particularly important. Heed Motorola's warning that the address ultimately latched may be some character clocks distant from when the strobe actually fired. In experience, on an IBM CGA card, this produces a cursor offset of 5-6 character clocks before calibration.
...
## Datasheet
 ...
 - (archive.org) [Motorola MC6845](https://archive.org/details/ibm_pc_datasheets/Controller%20Chips/Motorola%20MC6845%20CRTC/MC6845/)

## Primary References

 - (shaker.logonsystem.eu) [The Amstrad CPC CRTC Compendium V1.9](https://shaker.logonsystem.eu/)

### Additional Watching

 - (youtube.com) [Visiting Area 5150](https://www.youtube.com/watch?v=VWQB6HmqKig)